...
import {
  ...
  converter,
  getGithubBasePath,
} from '../../gatsby/utils/gatsbyRemark';
import { FILE_QL_NODE, SIPHON_QL_NODE, MARKDOWN_QL_NODE } from '../../__fixtures__/plugin-fixtures';
...
describe('Gatsby Remark Transform Path Converter Callback', () => {
  ...
  describe('Unit Tests', () => {
    ...
    it('converts only SourceDevhubGithub nodes', () => {
      ...
      const transformedPath = converter(astType, relativePath, MARKDOWN_QL_NODE, FILE_QL_NODE, {
        getNode,
      });
    ...
    it('returns a a transformed url', () => {
      const astType = 'image';
      const relativePath = '../images/banana.png';
      const transformedPath = converter(astType, relativePath, MARKDOWN_QL_NODE, SIPHON_QL_NODE, {
        getNode,
      });
