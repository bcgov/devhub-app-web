...
const hexGridCalculator = (hexArrayLength, pattern = [3, 2]) => {
    ...
    if(typeof hexArrayLength !== 'number' || hexArrayLength < 0) throw new Error('Hex Array Length must be greater than 0!');
    //is pattern an array of ints?
    if(pattern.constructor !== Array || pattern.some(isNaN) ) throw new Error('pattern must be an array of ints');
    ...
    for(let i = 0; i < hexArrayLength; i++) {
        ...
        if(currentCountForPattern >= patternNum) {
            ...
            patternPosition = patternPosition + 1 > patternNum ? 0 : patternPosition + 1;
