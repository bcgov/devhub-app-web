...
> Siphon is being deprecated in favor of third party source plugins that are now much more mature! :)
## Table of Contents
...
Siphon treats topics in the **registry** just like any other **source**. If the fetch queue detects
a registry item as a ***topic*** it grabs all the sources **within the topic**, extracts them,
maps the **name** and **other properties** from the topic registry item to them and pushes them into the fetch queue. In addition if the ***topic*** has a source property, **topicSource**, it will fetch a
markdown file to be used as content for the topic page at run time. 

If an items is **not a topic** it is simply pushed into the fetch queue without further processing. 

##### So what is a topic then?
The difference between a 'topic' and a regular 'source' is that *sources within the topic
...
Is really just a **special case of a topic**. A ***Source Topic***.
Each individual node that is created from all of the content that is fetched from the above repo
shares the same topic name `My Repo`. In addition other data items are loaded by token this is
a github repo. Things like a 'description' is automatically loaded from your repository description as
denoted in github. 

For a 'custom topic', each item could potentially be a repo or an individual file. For all of the
...
Firstly notice how the **individual sources** within the topic do not have the `name` attribute (even if it were there it would be ignored in a topic). Instead, when processed in the `fetchQueue` routine, it inherits the properties set at the top. 
...
source, it took priority. In essence properties in the topic *cascade* down to the child sources, but
if the child sources have properties assigned they take priority (with the exception of name).

Now that we have processed topics and indvidual sources in the registry as the **same thing**
they can be processed exactly the same way :)

> More detailed information on topics and how to configure them can be found [here.](./registerRepo.md)
...
### Siphon's Topic Node GraphQL Schema
> last updated Jan 10th 2018
```javascript
    id,
    children, // a list of all child siphonNodes that are related to this topic
    name, // name of the topic
    type, // curated, source etc.
    description, // a description for the topic as annotated within the registry file
    internal: {
    ...
    metadata: {
        position {Array} // this is the position of the topic as found within the registry file
    }
```
...
```javascript
    ...
    parent {
        id // gatsby required attribute, this is normally null but in our case holds a reference to the devhubTopic id this node belongs too 
    } 
    path // path to the resource
    topic {
        name // name of the topic, this is directly related to the name attribute in the registry
    ...
    metadata: {
        ...
        // the position of the topic within the registry file that this node belongs too
        // the position of the source within the topic that this node belongs too
        // the position of the node itself (also referred to as a resource) within the source
        // the below example says first topic, second source, fifth resource
...
- **default**: this will render your content out as well as setup navigation for any related resources that are apart of your source/topic
